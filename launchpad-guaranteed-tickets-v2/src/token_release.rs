...
pub const MAX_RELEASE_DIFF: u64 = 60 * 60 * 24 * 365 * 5; // 5 years
...
impl<M: ManagedTypeApi> UnlockSchedule<M> {
    ...
    fn validate(&self, current_timestamp: u64) -> bool {
        ...
        for milestone in self.milestones.iter() {
            if milestone.percentage > MAX_PERCENTAGE
                || milestone.release_timestamp < current_timestamp
                || milestone.release_timestamp < last_timestamp
                || milestone.release_timestamp > current_timestamp + MAX_RELEASE_DIFF
            {
                return false;
            }

            last_timestamp = milestone.release_timestamp;
    ...
    config::ConfigModule + launch_stage::LaunchStageModule + crate::events::EventsModule
{
    ...
    fn set_unlock_schedule(&self, unlock_milestones: MultiValueEncoded<MultiValue2<u64, u64>>) {
        ...
        for unlock_milestone in unlock_milestones {
            let (release_timestamp, percentage) = unlock_milestone.into_tuple();
        ...
        require!(
            unlock_schedule.validate(current_timestamp),
    ...
    fn compute_claimable_tokens(&self, address: &ManagedAddress) -> BigUint {
        ...
        for milestone in unlock_schedule.milestones.iter() {
            if milestone.release_timestamp <= current_timestamp {
