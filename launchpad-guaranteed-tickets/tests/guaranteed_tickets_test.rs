...
fn confirm_all_test() {
    ...
    lp_setup
        .select_base_winners_mock(NR_WINNING_TICKETS, 1)
        .assert_ok();
...
fn combined_scenario_test() {
    ...
    lp_setup.filter_tickets().assert_ok();
    lp_setup
        .select_base_winners_mock(NR_WINNING_TICKETS, 2)
        .assert_ok();
...
#[test]
fn add_more_guaranteed_tickets_scenario_test() {
    ...
    lp_setup
        ...
        .execute_tx(
            ...
            |sc| {
                let mut args = MultiValueEncoded::new();
                args.push((managed_address!(&new_participant), 1).into());
                args.push((managed_address!(&second_new_participant), MAX_TIER_TICKETS).into());
    ...
    // add migration guaranteed tickets for 2 users
    lp_setup
        .b_mock
        .execute_tx(
            &lp_setup.owner_address,
    ...
    lp_setup.confirm(&participants[4], 3).assert_ok();
    ...
    lp_setup
        .select_base_winners_mock(nr_winning_tickets, 4)
        .assert_ok();
    ...
    lp_setup
        ...
        .execute_tx(
            ...
            |sc| {
                ...
                assert_eq!(sc.ticket_status(2).get(), WINNING_TICKET); // migration guaranteed ticket -> additional_winning_tickets
                assert_eq!(sc.ticket_status(3).get(), false);
                assert_eq!(sc.ticket_status(4).get(), false);
